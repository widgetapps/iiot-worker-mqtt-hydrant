...
exports.handlePartData = function (redisClient, type, amqp, topicId, data, clientId) {
    ...
    util.log_debug(clientId, 'Does the header exist for key ' + key + '? ' + redisClient.hexists(key, 'header'));
    if (!redisClient.hexists(key, 'header')) {
        redisClient.hset(key, 'header', JSON.stringify(header));
        util.log_debug(clientId, 'Set REDIS header for key ' + key + ' value: ' + redisClient.hget(key, 'header'));
    }
    redisClient.hset(key, 'field_' + data.part[0], JSON.stringify(data.value));
    util.log_debug(clientId, 'Set REDIS field_' + data.part[0] + ' for key ' + key + ' value: ' + redisClient.hget(key, 'field_' + data.part[0]));

    util.log_debug(clientId, 'Length of the hash at key ' + key + ': ' +redisClient.hlen(key));
