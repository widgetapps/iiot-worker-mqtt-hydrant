        ...
        .exec(function (err, device) {
            ...
            amqp.then (function(conn) {
                ...
                assetPromise.then(function (asset) {
                    // Create and insert the Event doc to get the _id. This will be missing a bunch of fields
                    let eventDocument = new Event({
                        asset: asset._id,
                        device: device._id,
                    ...
                    eventDocument.save(function (err, savedEvent) {
                        ...
                        let docPromise = buildPartDocs(redisClient, hashLength, key, type, asset, device, savedEvent._id, clientId);
                        docPromise.then(function (result) {
                            ...
                            return ok.then(function() {
                                // Add documents dependant fields to the saved event doc,
                                savedEvent.start = new Date(documents[0].timestamp / 1000);
                                savedEvent.end = new Date(documents[documents.length - 1].timestamp / 1000);
                                savedEvent.count = documents.length;
                                savedEvent.description= sensor.typeString + ' event';
                                savedEvent.tag= {
                                ...
                                savedEvent.sensor= sensor._id;

                                savedEvent.save(function (err, finalEvent) {
                                    if (err) {
                                        console.log('DB Error: ' + err);
                                    }
