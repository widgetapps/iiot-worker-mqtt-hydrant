...
function buildPartDocs (redisClient, hashLength, key, type, asset, device, eventId, clientId) {
    ...
    redisClient.hget(key, 'header', function (err, header) {
        ...
        redisClient.hgetall(key, function (err, data) {
            util.log_debug(clientId, 'All data received. Length of data array returned: ' + data.length);
            for (let i = 0; i < data.length; i+=2) {
                unorderedValues[data[i]] = data[i+1];
            }

            _.forEach(unorderedValues, function (key, value) {
